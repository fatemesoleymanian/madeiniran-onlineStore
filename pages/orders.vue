                        ...
                        <div :id="`my-account${i}`" class="panel-collapse collapse" data-bs-parent="#faq">
                          ...
                          <div class="panel-body">
                            <div class="myaccount-info-wrapper">
                              ...
                              <div class="row">
                                <div class="col-lg-12 col-md-12" v-for="(d,j) in details" :key="j"
                                     v-if="details.length && !loader">
                                  <div class="billing-info d-flex justify-content-between align-items-center">
                                    <img :src="'https://api.madein-iran.com/public'+d.product.image"
                                         :alt="d.product.name"
                                    ...
                                    <h5 style="font-family:persianNumber;">ظرفیت : {{ d.state.type }}</h5>
                                    <h5 style="font-family:persianNumber;">
                                      {{ d.state.discounted_price.replace(/\B(?=(\d{3})+(?!\d))/g, ',') }} تومان</h5>
                                    <h5 style="font-family:persianNumber;"> به تعداد {{ d.count }}</h5>
                                  </div>
                                </div>
                              </div>
                              <div class="billing-back-btn float-start">
                                <div class="billing-btn">
                                  <h5 style="font-family:persianNumber;">
                                    {{ total.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',') }} تومان</h5>
                                </div>
                              </div>
