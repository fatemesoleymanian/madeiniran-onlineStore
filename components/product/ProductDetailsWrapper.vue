                  ...
                  <swiper :options="swiperOptionTop" ref="swiperTop">
                    <div class="large-img swiper-slide">
                      <img
                        class="img-fluid"
                        :src="
                          'https://api.madein-iran.com/public' + product.image
                        "
                        :alt="product.name"
                      />
                    </div>
            ...
            <div class="col-12 d-none d-md-block product-details-content">
              <div class="pro-details-quality justify-content-end">
                <div class="pro-details-wishlist">
                  <button
                    ...
                    title="لیست علاقمندیها"
                  >
                    <i
                      :class="
                        checkIsLiked === true ? 'fa fa-heart' : 'fa fa-heart-o'
                      "
                      style="color: darkred; font-size: 22px; display: inline"
                    ></i>
                  </button>
                </div>
              </div>
              <div class="pro-details-size-wrap" v-if="states.length">
                ...
                <div class="pro-details-size-content">
                  <label
                    class="radio"
                    v-for="(item, index) in product.state"
                    :key="index"
                  >
                    <input
                      type="radio"
                      name="sizeGroup"
                      @click="priceByState(index)"
                      checked
                    />
                    <span
                      class="check-mark"
              ...
              <div class="pro-details-meta pt-30">
                ...
                <ul>
                  <li>
                    <n-link :to="`/production-lines?category=`">{{
                      category
                    }}</n-link>
                  </li>
                </ul>
              ...
              <div class="pro-details-meta pt-20">
                ...
                <ul>
                  <li v-for="(tag, index) in product.tag" :key="index">
                    <n-link :to="`/production-lines?tag=`"
                      >{{ tag.name }},</n-link
                    >
                  </li>
                </ul>
        ...
        <div class="col-lg-6 col-md-6">
          <div class="product-details-content ml-70">
            ...
            <div class="product-details-price" v-if="auth && states.length">
              <span style="font-family: persianNumber">
                {{ discounted_price.replace(/\B(?=(\d{3})+(?!\d))/g, ",") }}
                تومان
              </span>
              <span
                class="old"
            ...
            <h6 v-if="!auth">
              <a
                href="/login-register"
                style="color: #0b4dfc; text-decoration: underline"
            ...
            <div class="pro-details-size-color">
              <button
                @click="navigateToRepresentationForm"
                class="btn btn-primary text-white"
                id="form-leader"
              >
                <a href="#description" style="font-size: 13px; color: white">
                  مشاوره رایگان</a
                >
              ...
              <div
                class="pro-details-size-wrap d-md-none d-block"
                v-if="states.length"
              >
                <h6 class="label">ظرفیت ها :</h6>
                <div class="pro-details-size-content">
...
export default {
  ...
  computed: {
    checkIsLiked() {
      ...
      if (this.$store.state.wishlist.find((el) => this.product.id === el.id))
        return true;
      else return false;
    },
  },
  ...
  methods: {
    ...
    async addToWishlist(product) {
      if (!localStorage.getItem("116111107101110"))
        return (window.location = "/login-register");
      // for notification
