        ...
        >
          <div class="shopping-cart-img">
            <n-link :to="`/product/${product.product.slug}`">
              <img
                :src="
                  'https://api.madein-iran.com/public' + product.product.image
                "
          ...
          <div class="shopping-cart-title">
            ...
            <span style="font-family: persianNumber">
              {{
                (product.count * product.state.discounted_price)
                  .toString()
                  .replace(/\B(?=(\d{3})+(?!\d))/g, ",")
              }}
              تومان</span
  ...
  computed: {
    ...
    total() {
      ...
      for (let i in this.$store.getters.getCart) {
        ...
        let item =
          this.$store.getters.getCart[i].state.discounted_price *
          this.$store.getters.getCart[i].count;
        // console.log(item)
        total += item;
      }
